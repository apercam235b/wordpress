...
class KeyController
{
    ...
    public static function RegisterKey($key, $is_registration = false)
    {
        try {
            ...
            if (!$is_valid) {
                throw new KeyException(__("Invalid License Key. Please check that the license key was entered correctly.", 'superbaddons'));
            }
            ...
            if (!is_array($response) || is_wp_error($response) || $response_code !== 200) {
                if ($response_code === 404) {
                    throw new KeyException(__("License key could not be validated. Please check that the license key was entered correctly.", 'superbaddons'), true, $response_code);
                } else {
                    throw new KeyException(__("Unable to validate license key. Please contact support for assistance.", 'superbaddons'), false, $response_code);
                }
            }
            ...
            if (!isset($data->level) || !isset($data->active) || !$data->active || !isset($data->expired) || !isset($data->verification) || !$data->verification || !isset($data->verification->exceeded) || !isset($data->verification->verified) || !isset($data->verification->stamp)) {
                throw new KeyException(__("License key not currently active. Please contact support for assistance.", 'superbaddons'));
            }

            if ($data->verification->exceeded) {
                throw new KeyException(__("You have already used up all your domain activations for this license key.", 'superbaddons'));
            }

            if ((!$data->verification->verified || !$data->verification->stamp) && $data->active && !$data->expired) {
                throw new KeyException(__("License key verification could not complete. Please contact support for assistance.", 'superbaddons'));
            }

            if ($data->expired) {
                throw new KeyException(__("License key has expired. Please renew your subscription to re-activate your license.", 'superbaddons'));
            }

            try {
                $option_controller->UpdateKey($key, $data->verification->stamp);
                self::UpdateKeyType($data->level, $data->active, $data->expired);
                return array("type" => $data->level, "active" => $data->active, "expired" => $data->expired, "verified" => $data->verification->verified);
            } catch (OptionException $o_ex) {
                self::RemoveKey($key, $data->verification->stamp);
                throw new KeyException(__("Unable to store license in WordPress. If the problem persists, please contact support.", 'superbaddons'));
            }
        } catch (KeyException $k_ex) {
            throw $k_ex;
        } catch (Exception $ex) {
            LogController::HandleException($ex);
            throw new KeyException(__("Internal Error Occured During License Key Registration", 'superbaddons'));
        }
    }
    ...
    public static function GetUpdatedLicenseKeyInformation()
    {
        try {
            ...
            return array("type" => KeyType::FREE, "active" => true, "expired" => false, "verified" => true);
        } catch (KeyException $k_ex) {
            throw $k_ex;
        } catch (Exception $ex) {
            LogController::HandleException($ex);
    ...
    public static function HasPremiumKey()
    {
        $option_controller = new OptionController();
        ...
        return $has_premium && !$is_expired && $is_active && $is_verified;
    }
    ...
    public static function GetKeyTypeLabel($keytype)
    {
        switch ($keytype) {
            case KeyType::PREMIUM:
                return __("Premium License", 'superbaddons');
            case KeyType::FREE_PLUS:
                return __("Free+ License", 'superbaddons');
            case KeyType::FREE:
            default:
                return __("Free License", 'superbaddons');
        }
    ...
    {
        $option_controller = new OptionController();
        return array("active" => $option_controller->KeyIsActive(), "expired" => $option_controller->KeyIsExpired(), "verified" => $option_controller->KeyIsVerified());
    }

    public static function UpdateKeyType($keytype, $active, $expired)
    {
        $option_controller = new OptionController();
        switch ($keytype) {
            case KeyType::PREMIUM:
        ...
        return $option_controller->UpdateKeyType($keytype, $active, $expired);
    }
}
